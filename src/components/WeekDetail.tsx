...
export default function WeekDetail() {
  ...
  const [wedFile, setWedFile] = useState<File | null>(null);
  ...
  async function submitReport(type: "wednesday" | "saturday") {
    ...
    const file = type === "wednesday" ? wedFile : satFile;
    ...
    try {
      ...
      if (type === "wednesday") { setWedReport(""); setWedFile(null); }
      else { setSatReport(""); setSatFile(null); }
      ...
      {activeTab === "reports" && (
        <div className="space-y-6">
          <ReportSection title="📅 Wednesday Mid-Week Report" type="wednesday"
            ...
            file={wedFile} setFile={setWedFile} onSubmit={() => submitReport("wednesday")}
          ...
          <ReportSection title="📅 Saturday End-of-Week Report" type="saturday"
            ...
            file={satFile} setFile={setSatFile} onSubmit={() => submitReport("saturday")}
            isInvestor={isInvestor} onRate={(rid) => setFbTarget(rid)} />

          {fbTarget && (
            <div className="fixed inset-0 bg-black/30 dark:bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
              <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border border-gray-200/60 dark:border-gray-700/60 rounded-2xl p-6 shadow-2xl w-full max-w-md space-y-4 shadow-xl">
...
function ReportSection({ title, type, reportData, report, setReport, file, setFile, onSubmit, isInvestor, onRate }: {
  title: string; type: string; reportData?: WeekReport; report: string; setReport: (v: string) => void;
  file: File | null; setFile: (f: File | null) => void; onSubmit: () => void; isInvestor: boolean; onRate: (rid: string) => void;
}) {
  return (
    <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-gray-200/60 dark:border-gray-800/60 rounded-2xl shadow-sm p-5 space-y-3">
      ...
      {reportData ? (
        <>
          ...
          {reportData.file_url && <a href={reportData.file_url} target="_blank" className="text-xs text-blue-500">📎 View attachment</a>}
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onSubmit} disabled={!report} className="px-3 py-1.5 bg-gradient-to-r from-indigo-500 to-violet-500 hover:brightness-110 disabled:bg-blue-300 text-white text-xs rounded-lg">Submit</button>
          </div>
        </div>
      )}
    </div>
